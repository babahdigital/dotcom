...
$div_logo = '<div class="sh-header-builder-logo">';
    $div_logo.= '<a href="'. esc_url( home_url( '/' ) ) .'">';
        ...
        if( in_array( $header_logo_size, [ 'large', 'full' ] ) && in_array( $header_layout, [ 3, 7 ] ) ) :
            $logo_size = $header_logo_size;
        endif;

        $url = ( is_numeric( $header_logo ) && jevelin_get_small_thumb( $header_logo, $logo_size ) ) ? jevelin_get_small_thumb( $header_logo, $logo_size ) : $standard_url;
        $div_logo.= '<img src="'.esc_url( $url ).'" class="sh-header-builder-logo-standard" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" />';

        // sticky
        $url = ( is_numeric( $header_logo_sticky ) && jevelin_get_small_thumb( $header_logo_sticky, $logo_size ) ) ? jevelin_get_small_thumb( $header_logo_sticky, $logo_size ) : $url;
        $div_logo.= '<img src="'.esc_url( $url ).'" class="sh-header-builder-logo-sticky" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" />';
...
if( class_exists( 'WooCommerce' ) && $header_nav_cart_hidden != true ) :
    $self = '
